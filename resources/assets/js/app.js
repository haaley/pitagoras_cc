...
(function ($) {
    ...
    function initDeleteTarget() {
        $('.swal-dialog-target').append(function () {
            ...
            var title = $(this).data('dialog-title') ? $(this).data('dialog-title') : 'Excluir';
            var message = $(this).data('dialog-msg');
            var type = $(this).data('dialog-type') ? $(this).data('dialog-type') : 'warning';
            var cancel_text = $(this).data('dialog-cancel-text') ? $(this).data('dialog-cancel-text') : 'Cancelar';
            var confirm_text = $(this).data('dialog-confirm-text') ? $(this).data('dialog-confirm-text') : 'Confirmar';
    ...
    function initComment() {
        ...
        form.on('submit', function () {
            ...
            submitBtn.val('Enviar...').addClass('disabled').prop('disabled', true);
            $.ajax({
                method: 'post',
                ...
                submitBtn.val("responder").removeClass('disabled').prop('disabled', false);
            });
            return false;
        });
    }
        ...
        if (projects.length > 0) {
            $.get('https://api.github.com/users/' + XblogConfig.github_username + '/repos?type=owner',
                function (repositories) {
                    if (!repositories) {
                        projects.html('<div><h3>Falha ao carregar</h3><p>Atualize e tente mais tarde...</p></div>');
                        return;
                    }
                    projects.html('');
