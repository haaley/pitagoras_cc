...
class AuthController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'name.regex' => "Nome do usuário somente aceita letras, numeros ou -,_",
            'name.unique' => "Nome de usuário  '$name'  já está cadastrado, se é você e se esqueceu da sua senha tente recuperação de senha",
            'email.unique' => "Email  '$email'  já está cadastrado, se é você e se esqueceu da sua senha tente recuperação de senha",
        ]);
        ...
        if ($this->bindGithub($user, $githubData)) {
            ...
            return redirect()->route('post.index')->with('success', 'O registro com Github foi um sucesso');
        } else {
            session()->forget('githubData');
            return redirect()->route('post.index')->with('success', 'O registro com Github falhou');
        }
    }
        ...
        if (auth()->check()) {
            ...
            if ($currentUser->github_id) {
                ...
                if ($currentUser->github_id == $githubUser->id) {
                    ...
                    return redirect()->route('post.index')->withErrors('Desculpe, vc usou uma conta diferente para logar com o github!');
                }
            } /*当前用户没有绑定Github账号，试图绑定*/
            else {
                /*返回的Github账号已经被绑定了，返回错误信息*/
                if ($user) {
                    return redirect()->route('post.index')->withErrors('Desculpe, esta conta de github foi usado por outra conta, era você?');
                } /*返回的Github账号没有被绑定，正常绑定*/
                else {
                    if ($this->bindGithub($currentUser, $this->getDataFromGithubUser($githubUser))) {
                        return redirect()->route('post.index')->with('success', 'Conectado com o github');
                    }
                    return redirect()->route('post.index')->withErrors('Não foi possível conectar com o github');
                }
            }
        } /*用户没有登陆*/
        else {
            ...
            if ($user) {
                auth()->loginUsingId($user->id);
                return redirect()->route('post.index')->with('success', 'Login bem-sucedido
');
            } /*一个全新的用户来了！！！尝试注册*/
            else {
