...
class CommentController extends Controller
{
    ...
    public function update(Request $request, Comment $comment)
    {
        ...
        if ($this->commentRepository->update($request->get('content'), $comment)) {
            ...
            if ($redirect)
                return redirect($redirect)->with('success', 'Modificado com sucesso');
            return back()->with('success', 'Modificado com sucesso');
        }
        return back()->withErrors('Modificação falhou');
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!auth()->check()) {
            if (!($request->get('username') && $request->get('email'))) {
                return response()->json(
                    ['status' => 500, 'msg' => 'Nome de usuário e email já não podem estar vazios!']
                );
            }
            $pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
            if (!preg_match($pattern, request('email'))) {
                return response()->json(
                    ['status' => 500, 'msg' => 'Email inválido !']
                );
            }
        }
    ...
    public function destroy($comment_id)
    {
        ...
        return back()->withErrors('Exclusão falhou');
    }
}
