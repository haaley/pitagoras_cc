...
abstract class Repository
{
    ...
    public function getMetaAll($meta, $model)
    {
        $modelQtd = count($model);
        ...
        switch ($modelQtd) {
            ...
            case 1 :
                $model[0]
                    ? $metaModelData[] = $v[] = unserialize(json_decode($model[0]->getMeta($meta)))
                    : $metaModelData[] = $v[] = unserialize(json_decode($model->getMeta($meta)));
                return $metaModelData;
                break;

            default :
                foreach ($model as $m) {
                    $metaModelData[] = $v[] = unserialize(json_decode($m->getMeta($meta)));
                }
                return $metaModelData;
                break;
        }
