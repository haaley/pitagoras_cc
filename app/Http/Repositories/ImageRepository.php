...
class ImageRepository extends FileRepository
{
    ...
    public function uploadImageToLocal(Request $request, $type = null, $getLocalUrl = false)
    {
        $file = $request->file('image');
        ...
        if ($path) {
            $name = $type
                ? $type . '_' . strtolower(str_replace(' ', '', auth()->user()->name))
            ...
            !$getLocalUrl ? $result = $image->id : $result = $image->key;
        } else {
            $result = false;
        }
        $this->clearCache();
